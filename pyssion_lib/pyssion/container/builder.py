...
class PVCBuilder:
    ...
    @staticmethod
    def calculate_size(project_dir: str, buffer: float = 1.2, minimum: str = "128Gi") -> str:
        project_dir = Path(project_dir)
        total_bytes = sum(f.stat().st_size for f in project_dir.rglob("*") if f.is_file())
...
class PVCManager:
    ...
    def ensure_or_resize(self, new_pvc: client.V1PersistentVolumeClaim):
        ...
        try:
            existing = self._core_v1.read_namespaced_persistent_volume_claim(name, self._namespace)
            ...
            if self._compare_size(requested_size, current_size) > 0:
                ...
                self._core_v1.patch_namespaced_persistent_volume_claim(
                    ...
                    body={"spec": {"resources": {"requests": {"storage": requested_size}}}}
                )
            else:
            ...
            if e.status == 404:
                ...
                self._core_v1.create_namespaced_persistent_volume_claim(self._namespace, new_pvc)
            else:
                raise
...
#Container Runner
def main_container_build(volume_name : str = None, image : str = "python", req_file : str = None, resources : client.V1ResourceRequirements = None,entrypoint_file : str = None,venv_cache : bool = False):
    if volume_name == None:
        raise
    ...
    cmd = command_build(req_file=req_file,entrypoint_file=entrypoint_file,venv_cache=venv_cache)
    ...
    container = client.V1Container(
        ...
        volume_mounts=[
            client.V1VolumeMount(mount_path="/app/code", name=volume_name)
        ]
    )

    return container

def command_build(req_file: str = None, entrypoint_file:str = "main.py",venv_cache:bool=False) -> str:
