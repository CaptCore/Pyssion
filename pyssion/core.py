...
from pyssion.runner.k8s_client import KubernetesJobCreator
...
class Pyssion(origin_pyssion):
    ...
    def run(self):
        ...
        print(job_launcher.build_job_spec())
    
    @error_wrapper
    def _create_configmap_from_code(self):
        ...
        for path in caller_dir.rglob("*"):
            if path.is_file() and path.stat().st_size < 1_000_000:
                ...
                key = rel_path.as_posix().replace("/", "_")
        ...
        try:
            self._core_v1.create_namespaced_config_map(
                namespace=self._k8s_config.get("namespace", "default"),
                body=configmap
            )
            print(f"🧩 ConfigMap '{self._unique_job_name}' created")
